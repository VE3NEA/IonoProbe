...
type
  ...
  public
    ...
    procedure LoadFileList(Mask: string; FindDirectories: boolean = false);
    procedure Subtract(Lst: TStrings);
    procedure Intersect(Lst: TStrings);
...
procedure TAlStringList.LoadFileList(Mask: string; FindDirectories: boolean = false);
var
  Sr: TSearchRec;
  rc: integer;
  Flag: integer;
begin
  Clear;
  if FindDirectories then Flag := faDirectory else Flag := 0;
  rc := FindFirst(Mask, Flag, Sr);
  try
    while rc = 0 do
      begin
      if (Sr.Name <> '.') and (Sr.Name <> '..') then  Add(Sr.Name);
      rc := FindNext(Sr);
      end;
  finally
    FindClose(sr);
...
procedure TAlStringList.Intersect(Lst: TStrings);
var
  i, Idx: integer;
begin
  Sorted := true;
  for i:=0 to Lst.Count-1 do
    if not Find(Lst[i], Idx) then Delete(Idx);
end;
